...
import static me.silva.guilherme.language.Language.ProptKey.*;
...
public class Window extends JFrame {
	...
	public Window() throws MalformedURLException {
		...
		langHand.addDependent(lang -> {
			sourceLabel.setText(lang.getPropt(SourceLabel));
			sourceLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			sorceField.setToolTipText(lang.getPropt(SourceToolTip));
			sorceField.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			sourceOpen.setText(lang.getPropt(SourceButton));
			sourceOpen.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			destLabel.setText(lang.getPropt(DestLabel));
			destLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			destField.setToolTipText(lang.getPropt(DestToolTip));
			sourceLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			destOpen.setText(lang.getPropt(DestButton));
			destOpen.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			patternTemplateLabel.setText(lang.getPropt(PatternTemplateLabel));
			patternTemplateLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			patternTemplateSelect.setToolTipText(lang.getPropt(PatternTemplateToolTip));
			patternTemplateSelect.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			patternLabel.setText(lang.getPropt(PatternCustomLabel));
			patternLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			patternField.setToolTipText(lang.getPropt(PatternCustomToolTip));
			patternField.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			patternHelp.setText(lang.getPropt(PatternCustomButton));
			patternHelp.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			optionsLabel.setText(lang.getPropt(OptionsLabel));
			optionsLabel.setFont(lang.getFont());
		});
		...
		transferModeSelect.setModel(new DefaultComboBoxModel(
				new String[] { "Copy files from source", "Move files from source (caution)" }));
		langHand.addDependent(lang -> {
			transferModeSelect.setToolTipText(lang.getPropt(TransferModeToolTip));
			transferModeSelect.setModel(new DefaultComboBoxModel(
					new String[] { lang.getPropt(TransferModeOption1), lang.getPropt(TransferModeOption2) }));
			transferModeSelect.setFont(lang.getFont());
		});
		...
		resolverModeSelect.setModel(new DefaultComboBoxModel(
				new String[] { "Add trailing index to files of same name ex. (1)", "Skip files with the same name" }));
		langHand.addDependent(lang -> {
			resolverModeSelect.setToolTipText(lang.getPropt(ResolverModeToolTip));
			resolverModeSelect.setModel(new DefaultComboBoxModel(
					new String[] { lang.getPropt(ResolverModeOption1), lang.getPropt(ResolverModeOption2) }));
			resolverModeSelect.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			extensionLabel.setText(lang.getPropt(ExtensionsLabel));
			extensionLabel.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			extensionField.setToolTipText(lang.getPropt(ExtensionsToolTip));
			extensionField.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			cancelButton.setText(lang.getPropt(CancelButton));
			cancelButton.setFont(lang.getFont());
		});
		...
		langHand.addDependent(lang -> {
			startButton.setText(lang.getPropt(StartButton));
			startButton.setFont(lang.getFont());
		});
		...
		startButton.addActionListener(e -> {
			...
			if (!Utility.contains(PATTERN_TEMPLATES, patternVal)) {
				...
				if (!Utility.requestConfirmation(
						langHand.getCurrent().getPropt(ConfirmationPopup1).replace("{pattern}", filledPattern))) {
					return;
				}
			}

			if (!Utility.requestConfirmation(
					langHand.getCurrent().getPropt(ConfirmationPopup2).replace("{total}", totalFileCount + ""))) {
				return;
			}

			if (dest.listFiles().length > 0
					&& !Utility.requestConfirmation(langHand.getCurrent().getPropt(ConfirmationPopup3))) {
				return;
			}
	...
	private String[] populateTemplates() {
		...
		for (int i = 0; i < populated.length; i++) {
			// populated[i] =
			// time.format(DateTimeFormatter.ofPattern(TEMPLATES[i])).replaceAll("NAME",
			// "Beach") + ".png";
	...
	private void updateStatus(int code) {
		status.setText(code == 0 ? "" : langHand.getCurrent().getPropt(values()[StatusCode1.ordinal() + code - 1]));
	}
}
